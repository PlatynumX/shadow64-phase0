#!/usr/bin/env python3
"""Build the compact Shadow64 R13 texture bank directly from the user's SW.GRP.

Tiles referenced by the selected map plus the R13 weapon/actor state frames are packed.
...
EXTRA_TILES = {
    # Sword HUD.
    2080, 2081, 2082, 2083,
    # Uzi HUD/fire and pickups.
    2004, 2006, 2008, 1797, 1799, 1807,
    # Health/armor and all key/card/skull-key variants used by the first slice.
    1802, 1803, 3030, *range(1765, 1781),
    # Basic Ninja/Coolie run, attack, pain and death frames used by R13.
    4096, 4162, 4116, 4219, 4227, 1400, 1420, 4268,
    # Switch art explicitly recognized by the R13 use layer.
    *range(551, 585), 2470, 2471,
}
REQUIRED_TILES = {2080, 2081, 2082, 2083, 2004, 2006, 2008}
...
def build_bank(grp: Path, map_name: str, output: Path, report: Path | None) -> None:
    ...
    if missing:
        ...
        required_missing = sorted(REQUIRED_TILES.intersection(missing))
        if required_missing:
            raise ValueError(f"required R13 gameplay tiles missing from ART: {required_missing}")
    ...
    if report:
        ...
        lines = [
            "Shadow64 R13 texture bank report",
            f"map={map_name}",
