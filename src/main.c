...
#define MAX_VISIBLE_SECTORS 48
#define MAX_PORTAL_DEPTH 6
#define RENDER_BLOCK 2
#define PLANE_BLOCK 4
#define DEPTH_W ((SCREEN_W + RENDER_BLOCK - 1) / RENDER_BLOCK)
#define DEPTH_H ((VIEW_H + RENDER_BLOCK - 1) / RENDER_BLOCK)
...
static uint16_t g_zbuf[DEPTH_W * DEPTH_H];
...
static float g_frame_scale = 1.0f;
static int g_last_visible_sector_count = 0;
static float g_cam_fx = 1.0f, g_cam_fy = 0.0f, g_cam_rx = 0.0f, g_cam_ry = 1.0f;
static int32_t g_floor_slope_q20[4096];
static int32_t g_ceil_slope_q20[4096];
static int16_t g_sector_sprite_head[4096];
static int16_t g_sprite_next[16384];
...
static bool load_texture_bank(const char *path, texture_bank_t *t) {
    ...
    for(uint32_t i=0;i<count;i++,d+=20){
        ...
        x->pixels=t->blob+x->data_offset;
        x->pow2w = x->w && ((x->w & (x->w - 1)) == 0);
        x->pow2h = x->h && ((x->h & (x->h - 1)) == 0);
        x->wmask = x->pow2w ? (uint16_t)(x->w - 1) : 0;
        x->hmask = x->pow2h ? (uint16_t)(x->h - 1) : 0;
        uint32_t sum=0,n=0; for(uint32_t k=0;k<x->data_size;k++){uint8_t q=x->pixels[k]; if(q!=255){sum+=q;n++;}} x->avg_index=(uint8_t)(n?sum/n:0);
    }
...
static uint8_t texel(const texture_t *t, int u, int v) {
    ...
    if(t->pow2w) u &= t->wmask; else { u%=t->w; if(u<0)u+=t->w; }
    if(t->pow2h) v &= t->hmask; else { v%=t->h; if(v<0)v+=t->h; }
...
static inline void raw_block(surface_t *s,int x,int y,int block,uint16_t c){
    int xmax=x+block; if(xmax>SCREEN_W)xmax=SCREEN_W;
    int ymax=y+block; if(ymax>VIEW_BOTTOM)ymax=VIEW_BOTTOM;
    if(x<0||y<VIEW_TOP||x>=SCREEN_W||y>=VIEW_BOTTOM)return;
    for(int yy=y;yy<ymax;yy++){uint16_t *row=(uint16_t*)((uint8_t*)s->buffer+(size_t)yy*s->stride);for(int xx=x;xx<xmax;xx++)row[xx]=c;}
}
static inline void zblock(surface_t *s,int x,int y,float z,uint16_t c){
    if((unsigned)x>=SCREEN_W||y<VIEW_TOP||y>=VIEW_BOTTOM)return;
    int bx=x/RENDER_BLOCK, by=(y-VIEW_TOP)/RENDER_BLOCK;
    if((unsigned)bx>=DEPTH_W||(unsigned)by>=DEPTH_H)return;
    uint16_t dz=(z>=65534.0f)?65534u:(z<=1.0f?1u:(uint16_t)z);
    int idx=by*DEPTH_W+bx;
    if(dz<g_zbuf[idx]){g_zbuf[idx]=dz;raw_block(s,bx*RENDER_BLOCK,VIEW_TOP+by*RENDER_BLOCK,RENDER_BLOCK,c);}
}

static void camera_basis(float*fx,float*fy,float*rx,float*ry);
static void init_slope_cache(void);
static void build_sector_sprite_lists(void);
...
static void init_slope_cache(void){
    memset(g_floor_slope_q20,0,sizeof(g_floor_slope_q20));
    memset(g_ceil_slope_q20,0,sizeof(g_ceil_slope_q20));
    for(int si=0;si<g_map.sector_count;si++){
        const map_sector_t *sec=&g_map.sectors[si];
        if(sec->wallnum<=0)continue;
        const map_wall_t *w=&g_map.walls[sec->wallptr], *w2=&g_map.walls[w->point2];
        int64_t dx=(int64_t)w2->x-w->x,dy=(int64_t)w2->y-w->y;
        double len=sqrt((double)(dx*dx+dy*dy));
        if(len<1.0)continue;
        if(sec->floorstat&2)g_floor_slope_q20[si]=(int32_t)(((double)sec->floorheinum*1048576.0)/(len*256.0));
        if(sec->ceilingstat&2)g_ceil_slope_q20[si]=(int32_t)(((double)sec->ceilingheinum*1048576.0)/(len*256.0));
    }
}
...
static int32_t sector_slope_z(int si, int32_t x, int32_t y, bool floor_plane) {
    ...
    int32_t factor = floor_plane ? g_floor_slope_q20[si] : g_ceil_slope_q20[si];
    if(!factor || sec->wallnum<=0)return base;
    const map_wall_t *w=&g_map.walls[sec->wallptr], *w2=&g_map.walls[w->point2];
    int64_t dx=(int64_t)w2->x-w->x,dy=(int64_t)w2->y-w->y;
    int64_t cross=dx*((int64_t)y-w->y)-dy*((int64_t)x-w->x);
    return base+(int32_t)((cross*(int64_t)factor)>>20);
}
...
static void update_camera_basis(void){float a=(float)g_player.angle*BUILD_PI/1024.0f;g_cam_fx=cosf(a);g_cam_fy=sinf(a);g_cam_rx=-g_cam_fy;g_cam_ry=g_cam_fx;}
...
static bool project_wall_segment(const map_wall_t*a,const map_wall_t*b,clipped_wall_t*out){
    float fx=g_cam_fx,fy=g_cam_fy,rx=g_cam_rx,ry=g_cam_ry;float dx1=(float)(a->x-g_player.x),dy1=(float)(a->y-g_player.y),dx2=(float)(b->x-g_player.x),dy2=(float)(b->y-g_player.y);
...
static void render_plane_background(surface_t*s){
    ...
    float fx=g_cam_fx,fy=g_cam_fy,rx=g_cam_rx,ry=g_cam_ry;
    int horizon=view_horizon();
    /* R13B: 4x4 plane tiles, cached camera basis and cached slope coefficients.
       This keeps floor/ceiling texturing visible while removing the bulk of the
       per-pixel FP/sqrt workload that made R13A crawl at 2-3 FPS. */
    for(int y=VIEW_TOP;y<VIEW_BOTTOM;y+=PLANE_BLOCK){
        int sample_y=y+(PLANE_BLOCK>>1);
        bool is_floor=sample_y>horizon;if(sample_y==horizon)sample_y++;
        const texture_t*t=is_floor?floor:ceil;
        int shade=shade_level(is_floor?sec->floorshade:sec->ceilingshade);
        int panx=is_floor?sec->floorxpanning:sec->ceilingxpanning,pany=is_floor?sec->floorypanning:sec->ceilingypanning;
        float denom=(float)(sample_y-horizon);
        if(fabsf(denom)<1.0f)denom=denom<0?-1.0f:1.0f;
        ...
        float base_depth=((float)(planez-g_player.z)/16.0f)*FOCAL/denom;if(base_depth<NEAR_PLANE)base_depth=NEAR_PLANE;
        bool sloped=is_floor?(sec->floorstat&2):(sec->ceilingstat&2);
        for(int x=0;x<SCREEN_W;x+=PLANE_BLOCK){
            int sample_x=x+(PLANE_BLOCK>>1);
            float depth=base_depth;
            float camx=((float)sample_x-160.0f)*depth/FOCAL;
            ...
            if(sloped){
                ...
                float d2=((float)(slopez-g_player.z)/16.0f)*FOCAL/denom;
                if(d2>NEAR_PLANE){depth=d2;camx=((float)sample_x-160.0f)*depth/FOCAL;wx=(float)g_player.x+fx*depth+rx*camx;wy=(float)g_player.y+fy*depth+ry*camx;}
            }
            uint8_t idx=t?texel(t,(int)(wx/64.0f)+panx,(int)(wy/64.0f)+pany):0;
            raw_block(s,x,y,PLANE_BLOCK,g_tex.palette16[shade][idx]);
        }
    }
}
...
static void draw_textured_span(surface_t*s,const texture_t*t,const map_wall_t*w,float z,float wall_t,int x,int y0,int y1,int full_top,int full_bottom){
    ...
    int span=full_bottom-full_top;if(span==0)span=1;
    int yy0=y0&~(RENDER_BLOCK-1);if(yy0<VIEW_TOP)yy0=VIEW_TOP;
    for(int y=yy0;y<=y1;y+=RENDER_BLOCK){int sample_y=y+(RENDER_BLOCK>>1);int v=t?(int)(((float)(sample_y-full_top)/(float)span)*(float)t->h*((w->yrepeat?w->yrepeat:8)/8.0f))+(int)w->ypanning:0;uint8_t idx=t?texel(t,u,v):0;if(idx==255&&((w->cstat&16)!=0))continue;zblock(s,x,y,z,g_tex.palette16[shade][idx]);}
}
...
static void render_wall(surface_t*s,const map_wall_t*w){
    ...
    float x1=sx1f,x2=sx2f,z1=c.z1,z2=c.z2,t1=c.t1,t2=c.t2;if(x1>x2){float q=x1;x1=x2;x2=q;q=z1;z1=z2;z2=q;q=t1;t1=t2;t2=q;}int ix1=(int)ceilf(x1),ix2=(int)floorf(x2);if(ix1<0)ix1=0;if(ix2>=SCREEN_W)ix2=SCREEN_W-1;if(ix1>ix2)return;ix1&=~(RENDER_BLOCK-1);const texture_t*tex=find_texture((uint16_t)w->picnum);
    for(int x=ix1;x<=ix2;x+=RENDER_BLOCK){
...
static void render_sprite(surface_t*s,const map_sprite_t*sp){
    ...
    if(!t||!t->w||!t->h)return;
    float fx=g_cam_fx,fy=g_cam_fy,rx=g_cam_rx,ry=g_cam_ry;float dx=(float)(sp->x-g_player.x),dy=(float)(sp->y-g_player.y),cx=dx*rx+dy*ry,z=dx*fx+dy*fy;if(z<=NEAR_PLANE)return;float sx=160.0f+cx*FOCAL/z;float worldw=(float)t->w*(float)(sp->xrepeat?sp->xrepeat:32)*0.5f,worldh=(float)t->h*(float)(sp->yrepeat?sp->yrepeat:32)*0.5f;float sw=worldw*FOCAL/z,sh=worldh*FOCAL/z;if(sw<1||sh<1||sw>800||sh>800)return;int left=(int)(sx-sw*0.5f),right=(int)(sx+sw*0.5f),bottom=project_y(sp->z,z),top=bottom-(int)sh;int shade=shade_level(sp->shade);
    if(left<0)left=0;
    if(right>=SCREEN_W)right=SCREEN_W-1;
    if(top<VIEW_TOP)top=VIEW_TOP;
    if(bottom>=VIEW_BOTTOM)bottom=VIEW_BOTTOM-1;
    if(left>right||top>bottom)return;
    left&=~(RENDER_BLOCK-1);top&=~(RENDER_BLOCK-1);
    for(int x=left;x<=right;x+=RENDER_BLOCK){int sample_x=x+(RENDER_BLOCK>>1);int u=(int)(((float)(sample_x-(int)(sx-sw*0.5f))/sw)*t->w);if(sp->cstat&4)u=t->w-1-u;for(int y=top;y<=bottom;y+=RENDER_BLOCK){int sample_y=y+(RENDER_BLOCK>>1);int v=(int)(((float)(sample_y-(bottom-(int)sh))/sh)*t->h);if(sp->cstat&8)v=t->h-1-v;uint8_t idx=texel(t,u,v);if(idx==255)continue;zblock(s,x,y,z,g_tex.palette16[shade][idx]);}}
}


static void build_sector_sprite_lists(void){
    for(int i=0;i<4096;i++)g_sector_sprite_head[i]=-1;
    for(int i=0;i<16384;i++)g_sprite_next[i]=-1;
    for(int i=0;i<g_map.sprite_count&&i<16384;i++){
        int si=g_map.sprites[i].sectnum;
        if(si<0||si>=g_map.sector_count)continue;
        g_sprite_next[i]=g_sector_sprite_head[si];
        g_sector_sprite_head[si]=(int16_t)i;
    }
}
static void render_world(surface_t*s){
    update_camera_basis();
    for(int i=0;i<DEPTH_W*DEPTH_H;i++)g_zbuf[i]=0xFFFFu;
    render_plane_background(s);
    sector_queue_t q[MAX_VISIBLE_SECTORS];
    int n=collect_visible_sectors(g_player.sector,q);g_last_visible_sector_count=n;
    ...
    for(int i=0;i<n;i++){int si=q[i].sector;for(int sp=g_sector_sprite_head[si];sp>=0;sp=g_sprite_next[sp])render_sprite(s,&g_map.sprites[sp]);}
}
...
static bool segment_blocked(int32_t x0,int32_t y0,int32_t x1,int32_t y1){
    /* R13B: LOS is only needed for portal-visible actors.  Walk those sectors'
       wall ranges directly instead of scanning all 3314 walls for every actor. */
    if(g_last_visible_sector_count>0){
        for(int si=0;si<g_map.sector_count;si++){
            if(!g_sector_visible[si])continue;
            const map_sector_t*sec=&g_map.sectors[si];
            for(int n=0;n<sec->wallnum;n++){
                const map_wall_t*w=&g_map.walls[sec->wallptr+n];
                if(!crosses_wall(x0,y0,x1,y1,w))continue;
...
static void sword_hit(void){
    ...
    for(int i=0;i<g_map.sprite_count;i++){
        map_sprite_t*sp=&g_map.sprites[i];if(sp->dead||sp->hp<=0)continue;if(g_last_visible_sector_count>0&&sp->sectnum>=0&&sp->sectnum<g_map.sector_count&&!g_sector_visible[sp->sectnum])continue;
...
static void update_enemies(void){
    ...
    for(int i=0;i<g_map.sprite_count;i++){
        map_sprite_t*sp=&g_map.sprites[i];if(sp->dead||sp->hp<=0)continue;if(g_last_visible_sector_count>0&&sp->sectnum>=0&&sp->sectnum<g_map.sector_count&&!g_sector_visible[sp->sectnum])continue;if(sp->attack_cooldown){int dec=(int)ceilf(g_frame_scale);sp->attack_cooldown=sp->attack_cooldown>dec?sp->attack_cooldown-dec:0;}
        ...
        if(!cansee)continue;
        float inv=1.0f/(dist>1?dist:1);float speed=(is_coolie(sp)?42.0f:24.0f)*g_frame_scale;
...
static void update_input(void){
    ...
    int turn=in.stick_x;if(abs(turn)<6)turn=0;g_player.angle=(int16_t)(((int)g_player.angle+(int)((float)(turn/5)*g_frame_scale))&2047);
    int look=in.stick_y;if(abs(look)<7)look=0;g_player.pitch+=(int)((float)(look/7)*g_frame_scale);if(g_player.pitch<-48)g_player.pitch=-48;if(g_player.pitch>48)g_player.pitch=48;
    ...
    float mdx=(fx*forward+rx*strafe)*(float)speed*g_frame_scale,mdy=(fy*forward+ry*strafe)*(float)speed*g_frame_scale;
    int move_steps=(int)ceilf(g_frame_scale);if(move_steps<1)move_steps=1;if(move_steps>8)move_steps=8;
    for(int ms=0;ms<move_steps;ms++)try_move_player((int32_t)(mdx/(float)move_steps),(int32_t)(mdy/(float)move_steps));
    update_vertical(p.r,in.btn.b);fire_weapon(p.z,in.btn.z);
    int timer_dec=(int)ceilf(g_frame_scale);if(timer_dec<1)timer_dec=1;
    if(g_player.weapon_timer>0){g_player.weapon_timer-=timer_dec;if(g_player.weapon_timer<0)g_player.weapon_timer=0;}
    if(g_player.fire_cooldown>0){g_player.fire_cooldown-=timer_dec;if(g_player.fire_cooldown<0)g_player.fire_cooldown=0;}
    if(g_player.damage_flash>0){g_player.damage_flash-=timer_dec;if(g_player.damage_flash<0)g_player.damage_flash=0;}
    if(g_message_timer>0){g_message_timer-=timer_dec;if(g_message_timer<0)g_message_timer=0;}
...
static void render_frame(void){
    ...
    graphics_draw_text(s,8,8,"SHADOW64 R13B FASTPATH");
    ...
    int fps=(int)(display_get_fps()+0.5f);
    snprintf(line,sizeof(line),"SEC:%d FPS:%d VIS:%d STICK:LOOK C:MOVE",g_player.sector,fps,g_last_visible_sector_count);
    graphics_draw_text(s,8,216,line);
    graphics_draw_text(s,8,228,"Z:FIRE A:USE R:JUMP B:CROUCH D-L/R:WEAPON");
...
int main(void){debug_init_isviewer();debug_init_usblog();display_init(RESOLUTION_320x240,DEPTH_16_BPP,3,GAMMA_NONE,FILTERS_RESAMPLE);joypad_init();int dr=dfs_init(DFS_DEFAULT_LOCATION);if(dr!=DFS_ESUCCESS)snprintf(g_status,sizeof(g_status),"DFS ERROR: %s",dfs_strerror(dr));else if(load_map(MAP_PATH,&g_map)&&load_texture_bank(TEX_PATH,&g_tex))reset_player();uint32_t last_ticks=TICKS_READ();while(true){uint32_t now=TICKS_READ();int32_t dt_ticks=TICKS_DISTANCE(last_ticks,now);last_ticks=now;if(g_frame==0)g_frame_scale=1.0f;else{g_frame_scale=((float)dt_ticks*30.0f)/(float)TICKS_PER_SECOND;if(g_frame_scale<0.25f)g_frame_scale=0.25f;if(g_frame_scale>8.0f)g_frame_scale=8.0f;}if(g_map.walls&&g_tex.blob)update_input();render_frame();}return 0;}
