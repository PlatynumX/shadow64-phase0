...
static void render_plane_background(surface_t*s){
    ...
    /* R13A: shade floors/ceilings in 2x2 blocks.  The R13 implementation did
       two slope evaluations plus several floating-point operations for every
       one of ~62K view pixels.  This keeps the same projection model while
       quartering the expensive samples; the final 2x2 fill is just stores. */
    for(int y=VIEW_TOP;y<VIEW_BOTTOM;y+=2){
        ...
        for(int x=0;x<SCREEN_W;x+=2){
            ...
            uint8_t idx=t?texel(t,(int)(wx/64.0f)+panx,(int)(wy/64.0f)+pany):0;uint16_t c=g_tex.palette16[shade][idx];
            raw_pixel(s,x,y,c);if(x+1<SCREEN_W)raw_pixel(s,x+1,y,c);if(y+1<VIEW_BOTTOM){raw_pixel(s,x,y+1,c);if(x+1<SCREEN_W)raw_pixel(s,x+1,y+1,c);}
        }
    }
}
...
static bool segment_blocked(int32_t x0,int32_t y0,int32_t x1,int32_t y1){
    /* R13A: LOS is only needed for portal-visible actors.  Walk those sectors'
...
static void render_frame(void){
    ...
    graphics_draw_text(s,8,8,"SHADOW64 R13A PERF GAMEPLAY");
